...
const createWalletValidation = [
  body('walletType').isIn(['STANDARD', 'SOCIAL']).withMessage('Invalid wallet type'),
  body('metadata').optional().isObject().withMessage('Metadata must be an object'),
];

const createSocialWalletGoogleValidation = [
  body('provider').equals('GOOGLE').withMessage('Provider must be GOOGLE'),
  body('providerId').trim().isLength({ min: 1 }).withMessage('Provider ID is required'),
...
const createSocialWalletPhoneValidation = [
  ...
  body('providerId').isMobilePhone('any').withMessage('Valid phone number is required'),
  body('verificationCode').optional().isLength({ min: 4, max: 8 }).withMessage('Invalid verification code'),
  body('providerData').isObject().withMessage('Provider data is required'),
];
...
router.get('/:id', authenticateJWT, WalletController.getWalletById);

// Update wallet metadata
router.put('/:id', authenticateJWT, WalletController.updateWallet);
